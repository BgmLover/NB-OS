...
void test_sched()
{
    ...
    proc2->pcb.context=(context*)((unsigned int)proc2+sizeof(PCB));
    clean_context(proc2->pcb.context);
    proc2->pcb.context->epc=(unsigned int)(print_0_fun);init->context->sp=(unsigned int)init+PAGE_SIZE;
    proc2->context->sp=(unsigned int)proc2+PAGE_SIZE;
    unsigned int init_gp;
    asm volatile("la %0, _gp\n\t" : "=r"(init_gp));
    proc2->context->gp=init_gp;
    ...
    //设置pgd属性为默认属性——可写
    kernel_strcpy(proc2->pcb.name, "print_0");
...
{//未完成
    ...
    add_to_background_list(get_first_task(&pcbs));
    next_list=&high_list;
...
{//重新修改过，待调试
    ...
    if(!list_is_empty(&high_list))
    {
        ...
        // if(next->pcb->priority==HIGH_PRIORITY)
        // {//如果优先级是最高，那么允许在最高优先级队列中执行两个之间片
        //     next_list=&high_list;
        ...
        // {//否则按照正常处理
        //     if(next!=current)
        //     {//如果当前进程与下一个取到的进程不是同一个，那么正常处理
        //         next_list=&above_normal_list;
        //         timeslices=ABOVE_NORMAL_TIMESLICES;
        //     }
        //     else
        //     {//如果队列中只有一个进程且为最高优先级，那么第二次取出该进程时做特殊处理
        //         current=next;
        //         next_list=&above_normal_list;
    ...
    current=next;
    old->pcb->state=STATE_READY;
    current->pcb->counter=timeslices;
    current->pcb->state=STATE_RUNNING;
    current->pcb->priority=lower_priority;
...
unsigned int sched()
{
    ...
    if(--counter==0)
    {
        ...
        if(flag)
        {
            //要将current进程放入到合适的前台队列中,next_list全局变量在前台调度函数中进行修改
