...
void bootmem_insert_mminfo(struct bootmem mm, unsigned int start, unsigned int end, unsigned int type){
    unsigned int idnex;
...
void bootmem_remove_mminfo(struct bootmem mm, unsigned int index){
    unsigned int tmp;

    if(index >= mm->cnt_infos) return;
    for(tmp = (index+1); tmp!=mm->cnt_infos; ++tmp0){
...
void bootmem_init(){
    ...
    insert_mminfo(&mm, 0, (unsigned int)(end-1), _MM_KERNEL);
...
void bootmem_bootmap_info(unsigned char *msg) {
    ...
    kernel_printf("%s :\n", msg);
    for (index = 0; index < mm.cnt_infos; ++index) {
        kernel_printf("\t%x-%x : %s\n", mm.info[index].start, mm.info[index].end, mem_msg[mm.info[index].type]);
    }
}
...
unsigned char *bootmm_alloc_pages(unsigned int size, unsigned int type, unsigned int align) {
    ...
    res = find_pages(cnt, mm.last_alloc + 1, mm.max_pfn, align >> PAGE_SHIFT);
    if (res) {
        insert_mminfo(&mm, (unsigned int)res, (unsigned int)res + size - 1, type);
        return res;
    }

    res = find_pages(cnt, 0, mm.last_alloc_end, align >> PAGE_SHIFT);
    if (res) {
        insert_mminfo(&mm, (unsigned int)res, (unsigned int)res + size - 1, type);
        return res;
    }
...
// return 0 means error
unsigned int bootmem_split_mminfo(struct bootmem mm, unsigned int index, unsigned int split_start) {
...
unsigned int bootmem_free_pages(unsigned int start, unsigned int size){
    ...
    if(index == mm.cnt_infos){
        printk("bootmem_free_pages: not alloc space(%x:%x)\n", start, size);
        return 0; // error
    }
    ...
    if(mm.info[index].start_pfn==start){
        if(mm.info[index].end_pfn==(start+size-1)){
            bootmem_remove_mminfo(&mm, index);
        }
        else{
        ...
        if(mm.info[index].end_pfn==(start+size-1)){
            ...
            bootmem_split_pages(index, start);
            bootmem_split_pages(index+1, start+size);
            bootmem_remove_mminfo(&mm, index+1);
        }
    }
    return 1;
}